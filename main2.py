...
brain1=Brain_v2( discounting=1,lambda_w=0.1,lambda_t=0.9,alpha_w=0.05,alpha_t=1,model=None)
brain2=Brain_v2( discounting=1,lambda_w=0.1,lambda_t=0.9,alpha_w=0.05,alpha_t=1,model=None)
temp=config.savedir
config.savedir=None
if True:

    brain1.load_model(model_constructor=model_,model_params=config.model_params,dir=config.savedir,identifier=1,eps=98)
    brain2.load_model(model_constructor=model_, model_params=config.model_params,
                      dir=config.savedir,identifier=2,eps=98)
config.savedir=temp
...
track_loc_dir='/home/pooja/PycharmProjects/lux_ai/outputs/tracking/ct_build/'
#track_loc_dir=None
agent1 = agent_v2(player=0, action_space=act_space(default_board_dims=(12, 12)),environment=copy.deepcopy(env),brain=brain1,use_learning=True,track_loc=track_loc_dir)

agent2 = agent_v2(player=1, action_space=act_space(default_board_dims=(12, 12)),environment=copy.deepcopy(env),brain=brain2,use_learning=True,track_loc=None)
