...
from ..utils import DEBUG_MESSAGE, LOCAL_EVAL
...
class RLAgent:
    def __init__(self, obs, conf):
        ...
        if torch.cuda.is_available():
            if self.agent_flags.device == "player_id":
                device_id = f"cuda:{min(obs.id, torch.cuda.device_count() - 1)}"
            else:
        ...
        self.me = self.game_state.players[obs.id]
        self.opp = self.game_state.players[(obs.id + 1) % 2]
    ...
    def __call__(self, obs, conf, raw_model_output: bool = False):
        ...
        value = agent_output["baseline"].squeeze().numpy()[obs.id]
    ...
    def resolve_collision_detection(self, obs, agent_output) -> List[str]:
        ...
        my_flat_log_probs = {
            key: val[obs.id] for key, val in flat_log_probs.items()
        }
        my_flat_actions = {
            key: torch.flatten(
                val.squeeze(0).squeeze(0)[obs.id],
                start_dim=-3,
                end_dim=-2
            )
