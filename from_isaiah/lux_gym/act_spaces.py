...
from from_lux.game_objects import CityTile, Unit
...
RESOURCES = Constants.RESOURCE_TYPES.WOOD, Constants.RESOURCE_TYPES.COAL, Constants.RESOURCE_TYPES.URANIUM
...
class BasicActionSpace(BaseActSpace):
    ...
    def check_movement_transfer(self,no_move,unit,unit_type,masks,pos_to_unit_dict,my_id):
        outside_board=self.default_board_dims[0]
        for direction in DIRECTIONS:
            new_pos_tuple = unit.pos.translate(direction, 1)
            new_pos_tuple = new_pos_tuple.x, new_pos_tuple.y
            if new_pos_tuple not in no_move and max(new_pos_tuple)<outside_board and min(new_pos_tuple)>0:#no move logic captured
                masks[ACTION_MEANINGS_TO_IDX[unit_type][f"MOVE_{direction}"]]=1

            if new_pos_tuple in pos_to_unit_dict.keys():#transfer logic captured
                if pos_to_unit_dict[new_pos_tuple].id == my_id:
                    candidate=pos_to_unit_dict[new_pos_tuple]
                    if candidate.is_worker: max_limit=100
                    else :max_limit=2000

                    for resource in RESOURCES:
                        if unit.cargo.resources>0 and candidate.cargo.resources <max_limit:
                                masks[ACTION_MEANINGS_TO_IDX[unit_type][f"TRANSFER_{resource}_{direction}"]]=1
        return masks
    ...
    def get_action_mask(self,game_state,pos_to_unit_dict,pos_to_city_tile_dict,no_move,my_id,constraint):
        output_dict={}
        player=game_state.players[my_id]
        max_action=len(ACTION_MEANINGS['city_tile']),len(ACTION_MEANINGS['worker']),len(ACTION_MEANINGS['cart'])


        for ct in player.city_tiles:  # list [city,units|workers]
            unit_type='city_tile'
            masks = np.zeros(max_action[0])
            ...
            if ct.cityid not in output_dict.keys():
                output_dict[ct.cityid]={}
                output_dict[ct.cityid][(ct.pos.x,ct.pos.y)]=masks.copy()
            else:#case for multiple adjacent cities
                output_dict[ct.cityid][(ct.pos.x, ct.pos.y)] = masks.copy()

        for unit in player.units:
            if unit.is_worker():
                unit_type = "worker"
                masks = np.zeros(max_action[1])

            elif unit.is_cart():
                unit_type = "cart"
                masks = np.zeros(max_action[2])
            ...
            if unit.can_act():
                ...
                if unit.is_worker():
                    ...
                    if game_state.map.get_cell_by_pos(unit.pos).road > 0 and \
                            ((unit.pos.x,unit.pos.y) not in pos_to_city_tile_dict.keys()):
                        masks[ACTION_MEANINGS_TO_IDX[unit_type]["PILLAGE"]] =1
                    # Building a city - check that worker has >= the required resources and is not on a resource
                    if unit.can_build(game_state.map)  and ((unit.pos.x,unit.pos.y)  not in pos_to_city_tile_dict.keys()):
                        masks[ACTION_MEANINGS_TO_IDX[unit_type]["BUILD_CITY"]]= 1
                masks=self.check_movement_transfer(no_move, unit, unit_type, masks, pos_to_unit_dict, my_id)
            masks[0]=1#no op is valida action
            output_dict[unit.id] = masks.copy()
