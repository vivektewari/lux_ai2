...
Buffers = typing.Dict[str, typing.List[typing.Union[torch.Tensor,typing.List[str]]]]
...
def act(
    ...
    try:
        ...
        gym_env,model,runner = create_env(flags,actor_index)
        seed = actor_index ^ int.from_bytes(os.urandom(4), byteorder="little")
        #gym_env.seed(seed)
        env = environment.Environment(gym_env)
        env_output =env.initial()
        #agent_state = model.initial_state(batch_size=1)
        #obs_formated=env.observation_space.observation(env_output[0])#agent_output, unused_state = model(env_output, agent_state)
        agent_output=get_action(runner,env_output['frame'])
        agent_output={'policy_logits':0,'baseline':0,'action':1}
        agent_state={'dummy':0}
        while not env.gym_env.sub_environment.done:
            index =0 # free_queue.get()
            if index is None:
                break
            ...
            for t in range(flags.unroll_length):
                if  env.gym_env.sub_environment.done:break
                timings.reset()

                with torch.no_grad():
                    agent_output= agent_output #env.gym_env.sub_environment.agents['simp'].action(env_output['frame'])

                timings.time("model")

                env_output = env.step([['m u_1 n'],['m u_1 n']])

                timings.time("step")
                env_output['last_action']=0
                for key in env_output:
...
def train(flags):  # pylint: disable=too-many-branches, too-many-statements
    ...
    if flags.num_buffers == 'None':  # Set sensible default for num_buffers.
    ...
    model= create_env(flags,-1)

    #model = vivek_agent(env.observation_space.shape, env.action_space.n, flags.use_lstm)
    buffers = create_buffers(flags, tuple(flags.input_dim), 10)
    ...
    learner_model = create_env(flags,-1)
...
class VivekNet(nn.Module):
    def __init__(self, observation_shape, num_actions, use_lstm=False):
        super(VivekNet, self).__init__()
...
def get_action(runner, observation):
    ...
    action1, log1,info = runner[0].act(observation())
    action2, log2,info = runner[1].act(observation())
...
def create_env(config,actor_index=0):
    ...
    seed = actor_index ^ int.from_bytes(os.urandom(4), byteorder="little")
    print(actor_index)
    configuration = {"seed": seed, "loglevel": 1, "annotations": True, "width": 12, "height": 12}
